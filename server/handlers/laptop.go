...
//	@Router			/gif/laptop [post]
func CreateLaptopGif(ctx *gin.Context) {
	...
	laptopCreateCommand := utils.NewCommandBuilder(fastMode).Laptop(imageFilePath, outputPath)
	err = laptopCreateCommand.Run()
	if err != nil {
		laptopErr := fmt.Sprintf(messages.ExecCommandFailed, "Laptop")
		log.Error(laptopErr, "err", err)
	...
	if err != nil {
		laptopErr := fmt.Sprintf(messages.FailToGenerateSpecificGif, "Laptop")
		log.Error(laptopErr, "err", err)
		ctx.JSON(http.StatusInternalServerError, messages.FailToGenerateGif)
		return
	}
